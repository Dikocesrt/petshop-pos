...
const (
	...
	UnsupportedMediaTypeCode Code = "UnsupportedMediaType" // Represents an wrong media type.
...
func UnsupportedMediaType(message any) *Exception {
	return &Exception{
...
func Internal(message any, err error) *Exception {
	slog.Error(message.(string), "error", err)
	return &Exception{
